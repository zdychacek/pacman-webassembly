...
class GRun: public GBase
{
public:
	static GRun* Instance()
	{
		if (s_pInstance == 0) {
			s_pInstance = new GRun;
...
private:
	static GRun* s_pInstance;

	GRun(int w, int h, int bpp, bool fullscreen);
	GRun();
	GRun(const GRun&);
	GRun& operator=(const GRun&);
	virtual ~GRun();
