...
bool drawText(TTF_Font *font, SDL_Color color, const char *text, int x, int y) {
  SDL_Surface *text_surface = TTF_RenderText_Blended(font, text, color);
  SDL_Texture* texture = SDL_CreateTextureFromSurface(TheGRun::Instance()->getRenderer(), text_surface);

  SDL_Rect text_dest = { x, y, 0, 0 };
  SDL_QueryTexture(texture, nullptr, nullptr, &text_dest.w, &text_dest.h);
  SDL_RenderCopy(TheGRun::Instance()->getRenderer(), texture, nullptr, &text_dest);
